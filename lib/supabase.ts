...
export const supabase = createClient<MultiTenantDatabase>(supabaseUrl, supabaseAnonKey, {
  auth: {
    persistSession: true,
...
if (!useSupabase && isDevelopment) {
  ...
  console.log('To use real Supabase, set up your .env.local file with valid credentials')
} else if (!useSupabase) {
...
export async function updateLeadStatus(leadId: number, status: string): Promise<{ success: boolean; error: any }> {
  if (!supabase) {
    console.log('Supabase not initialized');
...
export async function updateLead(leadId: number, updates: Partial<Lead>): Promise<Lead | null> {
  if (!supabase) {
    console.log('Supabase not initialized');
    return null;
  }
...
export async function createLead(lead: Omit<Lead, 'lead_id' | 'created_at' | 'client_id'>): Promise<Lead | null> {
  if (!supabase) {
    console.log('Supabase not initialized');
    return null;
  }
...
export async function deleteLead(leadId: number): Promise<{ success: boolean; error: any }> {
  if (!supabase) {
    console.log('Supabase not initialized');
    return { success: false, error: { message: 'Database not available' } };
  }
